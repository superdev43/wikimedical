...
class EPKB_Add_Ons_Page {
	...
	public function display_add_ons_page() {
		...
		<!-- Admin Page Wrap -->
		<div id="ekb-admin-page-wrap" class="ekb-admin-page-wrap epkb-add-ons-container">   <?php

			/**
			 * ADMIN HEADER (KB logo and list of KBs dropdown)
			 */
			EPKB_HTML_Admin::admin_header( EPKB_HTML_Admin::get_admin_header_logo() );
		...
		<div id="epkb_debug_info_tab_page">
			...
			if ( EPKB_Utilities::is_advanced_search_enabled() ) {   ?>
				<section class="save-settings">    <?php
					...
					$form->submit_button( $button_text, 'epkb_enable_advanced_search_debug', 'epkb_enable_advanced_search_debug','' ,true ,'' ,'epkb-primary-btn' ); ?>
				</section>  <?php
			}   ?>
	...
	private static function show_addons_box() {
		...
		<div class="add_on_container">      <?php
			...
			self::add_on_product( array(
				...
				'desc'              => __( 'Import articles and categories into your knowledge base from CSV file. Migrate and move your articles from another KB.', 'echo-knowledge-base' ),
		...
		$views_config[] = [
			...
			'boxes_list' => array(
				...
				array(
					...
					'description' => __( 'Enable debugging when instructed by the Echo team.', 'echo-knowledge-base' ),
					'html' => self::display_debug_info( new EPKB_HTML_Elements() ),
				)
			),
		];
		...
		$views_config[] = [
			...
			'boxes_list' => array(
				...
				array(
					...
					'description' => __( 'Write DELETE in the below input box if you want to delete ALL Knowledge Base data including KB Articles, KB Categories, KB Tags and KB options.', 'echo-knowledge-base' ),
