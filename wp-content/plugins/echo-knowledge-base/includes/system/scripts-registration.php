...
function epkb_register_public_resources() {
	...
	$epkb_vars = array(
		...
		'article_preview_not_available' => esc_html__( 'The article preview is not available. Press the read more link to read the article.', 'echo-knowledge-base' ),
...
function epkb_load_front_end_editor() {
	...
	foreach ( EPKB_Typography::$google_fonts_10_links as $font_link ) { ?>
...
function epkb_add_admin_bar_button( WP_Admin_Bar $wp_admin_bar ) {
	...
	$title = epkb_front_end_editor_title( false );
	if ( ! empty( $title ) ) {
...
function epkb_front_end_editor_title( $with_dialog = true ) {
