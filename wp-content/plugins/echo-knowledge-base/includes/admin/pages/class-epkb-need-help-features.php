...
class EPKB_Need_Help_Features {
	...
	public static function get_page_view_config() {
		...
		return array(
			...
			'main_class' => in_array( 'features_tab_visited', $kb_flags ) ? 'epkb-admin__flag--visited' : '',
	...
	private static function get_feature_box( $feature ) {
		...
		<div class="epkb-kbnh__feature-container__col epkb-kbnh__feature__icon-col"><span class="<?php echo empty( $feature['icon'] ) ? $icon : $feature['icon']; ?>"></span></div>

		<div class="epkb-kbnh__feature-container__col epkb-kbnh__feature__content-col">
			<h3 class="epkb-kbnh__feature-name<?php echo $feature['plugin'] != 'core' && $feature['plugin'] != 'crel' && $feature['plugin'] != 'ep'.'hd' ? ' epkb-kbnh__feature-name--pro' : ''; ?>"><?php echo $feature['name']; ?></h3>   <?php
			...
			<div class="epkb-kbnh__feature-links">  <?php
				...
				if ( ! empty( $feature['config'] ) && EPKB_Utilities::is_plugin_enabled( $feature['plugin'] ) && ! EPKB_Core_Utilities::is_run_setup_wizard_first_time() && ( current_user_can( EPKB_Admin_UI_Access::get_admin_capability() ) || ( ! empty( $feature['min_capability'] ) && current_user_can( $feature['min_capability'] ) ) ) ) {   ?>
					<a class="epkb-kbnh__feature-link" href="<?php echo $feature['config']; ?>" target="_blank"><span><?php _e( 'Configure', 'echo-knowledge-base' ); ?></span></a>    <?php
				}

				if ( ! empty( $feature['docs'] ) ) {    ?>
					<a class="epkb-kbnh__feature-link" href="<?php echo $feature['docs']; ?>" target="_blank"><span><?php _e( 'Docs', 'echo-knowledge-base' ); ?></span></a>    <?php
				}

				// Link to Video Tutorial
				if ( ! empty( $feature['video'] ) ) {  ?>
					<a class="epkb-kbnh__feature-link" href="<?php echo $feature['video']; ?>" target="_blank"><span><?php _e( 'Video Tutorial', 'echo-knowledge-base' ); ?></span></a>    <?php
				}

				// if plugin is not enabled, then show Learn More
				if ( ! EPKB_Utilities::is_plugin_enabled( $feature['plugin'] ) ) {  ?>
					<a class="epkb-kbnh__feature-link" href="<?php echo self::get_plugin_sales_page( $feature['plugin'] ); ?>" target="_blank"><span><?php _e( 'Learn More', 'echo-knowledge-base' ); ?></span></a>    <?php
				}				?>
		...
		<div class="epkb-kbnh__feature-container__col epkb-kbnh__feature__status-col">    <?php
			...
			// Plugin is not enabled
				...
				if ( $feature['plugin'] == 'crel' ) {
					...
					echo '<a class="epkb-kbnh__feature-status epkb-kbnh__feature--disabled" href="' . self::get_plugin_sales_page( $feature['plugin'] ) . '" target="_blank"><span>' . __( 'Upgrade', 'echo-knowledge-base' ) . '</span></a>';
				}
			}   ?>
	...
	private static function get_features_config() {
		...
		return [
			...
			[
				...
				'category'  => 'advanced',
				'icon'      => '',
				'name'      => __( 'Articles CSV Import', 'echo-knowledge-base' ),
				'desc'      => __( 'Import articles and categories from the CSV file into your Knowledge Base.', 'echo-knowledge-base' ),
			...
			[
				...
				'desc'      => __( 'Easy and simple frontend Editor helps you change colors, labels, fonts, and styles in no time.', 'echo-knowledge-base' ),
				'config'    => '',
			...
			[
				...
				'category'  => 'search',
				'icon'      => '',
				'name'      => __( 'Search Shortcode', 'echo-knowledge-base' ),
			...
			[
				...
				'desc'      => __( 'Control access to private a Knowledge Base utilizing WordPress user accounts. Grant permissions using roles and groups.', 'echo-knowledge-base' ),
				'config'    => '',
	 ...
	 * @return String
	 */
	private static function get_plugin_sales_page( $plugin_name ) {
		switch( $plugin_name ) {
			case 'amgr':
			...
			case 'epie':
				return 'https://www.echoknowledgebase.com/wordpress-plugin/kb-import-export/';
			case 'crel':
	...
	private static function features_tab_footer() {
		...
		<span><?php _e( 'Cannot find a feature?', 'echo-knowledge-base' ); ?></span>
		<a href="https://www.echoknowledgebase.com/feature-request/" class="epkb-kb__wizard-link" target="_blank"><?php _e( 'Contact us', 'echo-knowledge-base' ); ?></a>   <?php
	...
	public static function features_tab_visited() {
		...
		$kb_flags = EPKB_Utilities::get_wp_option( 'epkb_flags', [], true );
		if ( ! in_array( 'features_tab_visited', $kb_flags ) ) {
			array_push( $kb_flags, 'features_tab_visited' );
