...
class EPKB_Site_Builders {
	...
	private function check_vc_cpt() {
		...
		if ( in_array( 'vc_notice_shown', $kb_flags ) ) {
			return true;
		}

		array_push( $kb_flags, 'vc_notice_shown' );
	...
	private function check_beaver_cpt() {
		...
		// show one time notice
		$kb_flags = EPKB_Utilities::get_wp_option( 'epkb_flags', [], true );
		if ( in_array( 'beaver_notice_shown', $kb_flags ) ) {
			return true;
		}

		array_push( $kb_flags, 'beaver_notice_shown' );
		EPKB_Utilities::save_wp_option( 'epkb_flags', $kb_flags, true );
