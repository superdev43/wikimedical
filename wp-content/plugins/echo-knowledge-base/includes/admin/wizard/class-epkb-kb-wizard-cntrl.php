...
class EPKB_KB_Wizard_Cntrl {
	...
	private function apply_theme_wizard_changes( $wizard_kb_id, $orig_config, $new_config ) {
		...
		if ( ! empty($update_kb_msg) ) {
			EPKB_Utilities::ajax_show_error_die( EPKB_Utilities::report_generic_error( 36, $update_kb_msg ) );
		}
	...
	private function apply_features_wizard_changes( $orig_config, $new_config ) {
		...
		$article_sidebar_component_priority = EPKB_Utilities::post( 'article_sidebar_component_priority', [] );
		if ( empty($article_sidebar_component_priority) || ! array( $article_sidebar_component_priority ) ) {
			EPKB_Utilities::ajax_show_error_die( EPKB_Utilities::report_generic_error( 173 ) );
		}
	...
	public function wizard_update_order_view() {
		...
		if ( $sequence_settings['articles_display_sequence'] == 'user-sequenced' || $sequence_settings['categories_display_sequence'] == 'user-sequenced' ) {
			$message =  esc_html__( 'Drag & Drop Elements for Sorting', 'echo-knowledge-base' );
		}
	...
	public function update_wizard_preview() {
		...
		wp_die( json_encode( array( 
			'message' => esc_html__( 'Preview Updated', 'echo-knowledge-base' ),
			'html' => $html
		) ) );
	}
	...
	public function hide_demo_content_alert() {
		...
		wp_die( json_encode( array( 
			'message' => esc_html__( 'Alert was hidden', 'echo-knowledge-base' ),
			) ) );
	}
