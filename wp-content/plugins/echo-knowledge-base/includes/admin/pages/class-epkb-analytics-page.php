...
class EPKB_Analytics_Page {
	...
	public function display_plugin_analytics_page() {
		...
		<!-- Admin Page Wrap -->
		<div id="ekb-admin-page-wrap" class="ekb-admin-page-wrap epkb-config-container epkb-analytics-container <?php do_action( 'eckb_add_container_classes'); ?>">    <?php

			/**
			 * ADMIN HEADER (KB logo and list of KBs dropdown)
			 */
			EPKB_HTML_Admin::admin_header( EPKB_HTML_Admin::admin_header_content( $this->kb_config, ['admin_eckb_access_search_analytics_read'] ) );
	...
	/**
	 * Get HTML for ASEA add-on ad box
	 *
	 * @return false|string
	 */
	private function get_asea_addon_ad_box_html() {
	...
	private function get_regular_views_config() {
			...
			$core_views[] = [
				...
				'boxes_list' => array(
					...
					array(
						...
						'html' => $this->get_asea_addon_ad_box_html(),
					),
				),
			];
		}
		...
		foreach ( $add_on_views as $view_index => $view ) {
			...
			if ( ! isset( $view['list_key'] ) || ! in_array( $view['list_key'], ['search-data'] ) ) {
				continue;
			}
