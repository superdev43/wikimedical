...
class EPKB_KB_Wizard_Global {
	...
	public function show_kb_urls_global_wizard() {
		...
		if ( $this->kb_config['id'] != EPKB_KB_Config_DB::DEFAULT_KB_ID && ! EPKB_Utilities::is_multiple_kbs_enabled() ) {
			echo '<div class="epkb-admin__warning-message">' . esc_html__( 'Ensure that Multiple KB add-on is active and refresh this page. ', 'echo-knowledge-base' ) . EPKB_Utilities::contact_us_for_support() . '</div>';
			return;
		}       ?>
		...
		<div id="eckb-wizard-global__page" class="eckb-wizard-global-page epkb-config-wizard-content" data-kb-main-page-id="<?php echo esc_attr( $this->main_page_id ); ?>">
			<div class="epkb-config-wizard-inner">      <?php
				...
				if ( ! $this->article_path_matches ) {    ?>
					<div class="epkb-admin__warning epkb-admin__warning--white-red"><?php esc_html_e( 'We detected that your KB URL has changed. When ready, please update your Articles URL below to match the KB URL:', 'echo-knowledge-base' ); ?></div>     <?php
				}   ?>
	...
	private function wizard_buttons_v2() {
		...
		<div class="epkb-wizard-content">
			<div class="eckb-wizard__warning-container">
				<div class="eckb-wizard__warning-wrap">
					<div class="epkb-admin__warning"><?php esc_html_e( "Warning: When you change the article's URL, ensure that you update or redirect any existing article links to the new URL.", 'echo-knowledge-base' ); ?></div>
				</div>
			</div>
		</div>
