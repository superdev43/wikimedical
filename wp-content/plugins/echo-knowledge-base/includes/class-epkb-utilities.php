...
class EPKB_Utilities {
	...
	public static function get_allowed_html_tags( $contexts=[] ) {
		...
		add_filter( 'safe_style_css', array( 'EPKB_Utilities', 'safe_inline_css_properties' ) );
		...
		// Audio
		if ( EPKB_Admin_UI_Access::is_user_allowed_unfiltered_html() && ( in_array( 'audio', $contexts ) || in_array( 'all', $contexts ) ) ) {
			$tags_list['audio']     = self::get_allowed_html_attributes( 'audio' );
			$tags_list['source']    = self::get_allowed_html_attributes( 'source' );
		}

		// Video
		if ( EPKB_Admin_UI_Access::is_user_allowed_unfiltered_html() && ( in_array( 'video', $contexts ) || in_array( 'all', $contexts ) ) ) {
			$tags_list['video']     = self::get_allowed_html_attributes( 'video' );
			$tags_list['source']    = self::get_allowed_html_attributes( 'source' );
		}

		// Form
		if ( EPKB_Admin_UI_Access::is_user_allowed_unfiltered_html() && ( in_array( 'form', $contexts ) || in_array( 'all', $contexts ) ) ) {
		...
		// Style
		if ( EPKB_Admin_UI_Access::is_user_allowed_unfiltered_html() && ( in_array( 'style', $contexts ) || in_array( 'all', $contexts ) ) ) {
			$tags_list['style']     = self::get_allowed_html_attributes( 'script' );
		}

		// Script
		if ( EPKB_Admin_UI_Access::is_user_allowed_unfiltered_html() && ( in_array( 'script', $contexts ) || in_array( 'all', $contexts ) ) ) {
			$tags_list['link']      = self::get_allowed_html_attributes( 'script' );
			$tags_list['script']    = self::get_allowed_html_attributes( 'script' );
		}

		// Iframe
		if ( EPKB_Admin_UI_Access::is_user_allowed_unfiltered_html() && ( in_array( 'iframe', $contexts ) || in_array( 'all', $contexts ) ) ) {
