	...
	public function show_admin_notices() {
		...
		foreach ( $notices as $key => $notice ) {
			...
			if ( $notice['type'] == 'large-notice' || $notice['type'] == 'large-info' ) { ?>
				<div
					class="epkb-notice epkb-notice-large-box notice notice-<?php echo esc_attr( $notice['type'] ); ?> notice-<?php echo esc_attr( $notice['id'] ); ?>">
					<div class="epkb-notice-icon"><?php echo wp_kses( $notice['icon'], EPKB_Utilities::get_allowed_html_tags( ['all'] ) ); ?></div>
					<div class="epkb-notice-text">
						<h3><?php echo wp_kses( $notice['title'], EPKB_Utilities::get_allowed_html_tags( ['all'] ) ); ?></h3>
						<p><?php echo wp_kses( $notice['text'], EPKB_Utilities::get_allowed_html_tags( ['all'] ) ); ?></p> <?php
						if ( ! empty( $notice['id'] ) ) { ?>
							<!-- TODO <a href="#" class="epkb-notice-remind epkb-notice-btn btn-green"><?php esc_html_e( 'Remind Me Later', 'echo-knowledge-base' ); ?></a> -->
							<a href="#" class="epkb-notice-dismiss epkb-notice-btn btn-grey" data-notice-id="<?php echo esc_attr( $notice['id'] ); ?>">
								 <?php esc_html_e( 'Dismiss', 'echo-knowledge-base' ); ?></a>                        <?php
						} ?>
					</div>
				</div>      <?php
			} else { ?>
				<div
					class="epkb-notice notice notice-<?php echo esc_attr( $notice['type'] ); ?> notice-<?php echo esc_attr( $notice['id'] ); ?>"
					style="display:block;">
					<p>                        <?php
						echo wp_kses( $notice['text'], EPKB_Utilities::get_allowed_html_tags( ['all'] ) );
						if ( ! empty( $notice['id'] ) ) { ?>
							<a href="#" class="epkb-notice-dismiss" data-notice-id="<?php echo esc_attr( $notice['id'] ); ?>"><?php esc_html_e( 'Dismiss', 'echo-knowledge-base' ); ?></a>                        <?php
						} ?>
					</p>
				</div>      <?php
			}
