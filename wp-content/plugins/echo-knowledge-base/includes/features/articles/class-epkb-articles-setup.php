...
class EPKB_Articles_Setup {
	...
	*/
	...
	private static function article_content_header_feature( $args, $feature ) {
		...
		switch( $feature ) {
			...
			case 'last_updated_date':
				 ...
				 self::meta_data_header_feature( $args, $feature, 'epkbfa-pencil-square-o', 'last_updated_date_icon_on', 'last_udpated_on_text', $date );
				break;
			case 'breadcrumb':
	...
	public static function table_of_content( $args ) {
		...
		$mobile_breakpoint = $args['config']['article-mobile-break-point-v2'];
		if ( is_numeric( $mobile_breakpoint ) && ! empty( $_REQUEST['epkb-editor-page-loaded'] ) ) {
			$mobile_breakpoint -= 400;
		}
		...
		$wrap = '
			<div class="eckb-article-toc ' . $classes . ' eckb-article-toc-reset "				
				...
				data-mobile_breakpoint="' . $mobile_breakpoint . '"
				>' . $title . '</div>
			';
